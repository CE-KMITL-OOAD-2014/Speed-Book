                    ...
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                        ...
                        <strong>   
                          <?php 
                            ...
                            $result=mysql_query("SELECT*FROM login WHERE email LIKE '$user%' AND password LIKE '$key%' ",$db)or die("กรุณาล๊อกอินด้วยรหัสผ่านที่ถูกต้อง");
                            ...
                            <div class="navbar-login">
                                <div class="row">
                                    ...
                                    <div class="col-lg-8">
                                        ...
                                        <input type="text" id="user" name="user" class="login_input" value=<?if(isUser($user,$key))echo $_GET["user"];?> >
                                        </p>
...
<?php 
    ...
    $db=mysql_connect("localhost","root","password"); 
                            ...
                            $max=0;
                            $idBook;
                             $rsLogin=select("login","email LIKE '$user'");
                             if($rsLogin[book1]!="0"){$idBook[]=$rsLogin[book1];$max++;}
                            if($rsLogin[book2]!="0"){$idBook[]=$rsLogin[book2];$max++;}
                             //$max=$rs[size];
                             for($i=0;$i<=$max;$i++){
                              $rs=select("book","id LIKE '$idBook[$i]'");
                              $download[]=$rs[name];
...
return @mysql_fetch_array($objQuery);
    ...
    ?>
     <a href="<? if($max==0)echo "file/".$download[1]; ?>"  class="thumbnail">
      <img data-src="holder.js/100%x180" alt="" src="<? if($max==0)echo "image/defaultimg.png"?>">
    <?if($max==0) echo"</a>"; ?>
  <?if($max==0)echo "</div>" ?>
  <div class="col-xs-6 col-md-3">
...
function isUser($user,$key){
    ...
    $result=mysql_query("SELECT*FROM login WHERE email LIKE '$user%' AND password LIKE '$key%' ",$db)or die("กรุณากรอกรหัสที่ถูกต้อง");
     $rs=mysql_fetch_array($result)or die("กรุณากรอกรหัสผ่านที่ถูกต้อง");
 $n=$rs[username];
